...
import { DOMParser } from 'xmldom'; // Para parsear XML en el navegador
...
const KmlUploader: React.FC<Props> = ({ proyectoId }) => {
    ...
    const handleUpload = async () => {
        ...
        const geojson = toGeoJSON.kml(kmlDoc) as any; // convierte KML a GeoJSON
        ...
        for (const feature of geojson.features) {
            ...
            await createLote(lote); // POST al backend
        }
...
function geojsonFeatureToWKT(feature: any) {
    ...
    if (feature.geometry.type === 'Polygon') {
        // WKT POLYGON: "POLYGON ((x1 y1, x2 y2, ...))"
...
function geojsonCentroidToWKT(feature: any) {
    ...
    // Simple centroid aproximado para un polígono
    if (feature.geometry.type === 'Polygon') {
        const ring = coords[0];
        let sumX = 0, sumY = 0;
